...
class GenerateParaRefsForDocx:
    ...
    def get_bullet_number(self, paragraph: Paragraph) -> tuple[str, int, str]:
        ...
        if fmt_level == "decimal":
            ...
            current_val = to_letter(n)
        elif fmt_level == "lowerLetter":
            current_val = to_letter(n, lowercase=True)
        elif fmt_level == "upperRoman":
            current_val = to_roman(n)
        elif fmt_level == "lowerRoman":
            current_val = to_roman(n, lowercase=True)
        else:
            current_val = str(n)

        # Build the bullet: if level 0, just use current value; otherwise append to parent
        if ilvl_int == 0:
            bullet = current_val
        else:
