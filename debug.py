...
def build_hierarchical_number(bullet_info, hierarchical_levels):
    ...
    # Pop any levels that are same or deeper (this fixes A, B, C issue)
    while len(hierarchical_levels) >= lvl:
        hierarchical_levels.pop()

    # Add current level value
    hierarchical_levels.append(bullet_info["value"])

    # Join into hierarchical numbering string
